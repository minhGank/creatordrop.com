...
Phase 15 adds a separate browser-compatible persisted-opening verifier entry point. It accepts the authoritative proof response only when the seed is revealed, independently recomputes HMAC rounds from round zero until the accepted digest, and compares the accepted round/digest, selection value, immutable manifest association, reward version, and position. Persisted opening proofs do not accept client-supplied rejected-round digests. The existing vector verifier and its complete `roundDigests` checks remain unchanged.
