...
} from '../src/modules/leaderboards/leaderboard.service.js';
...
describe('public leaderboard reads', () => {
  it('is publicly readable and exposes username rather than the internal user UUID', async () => {
    const authenticateHandler: RequestHandler = (_request, _response, next) => {
      next();
    };
    const authenticate = vi.fn(authenticateHandler);
    ...
    const response = await request(createTestApp({ authenticate, leaderboardService: service }))
